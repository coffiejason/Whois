...
function LookupDomain($domain){
	global $whoisservers;
	$domain_parts = explode(".", $domain);//splits the string 
	$tld = strtolower(array_pop($domain_parts));//sets string array to lower case and pop elements off the end of the arry
	$whoisserver = $whoisservers[$tld];//checks if input string is in the whois server list
	if(!$whoisserver) {
		return "Error: No appropriate Whois server found for $domain domain!";
	}
	$result = QueryWhoisServer($whoisserver, $domain);//passes string to a function for server query
	if(!$result) {
...
<body align='center'>
<h5>By Group 10<h5>
<h1 style = 'color: red;'align='center'>WHOIS SEARCH</h1>



<form action="<?=$_SERVER['PHP_SELF'];?>" align='center'>
<p><b><label for="domain"></label></b> <input style='height: 40px; margin-left:0px; width:500px;' placeholder = 'Domain/IP Address' type="text" name="domain" id="domain" value="<?=$domain;?>"></input> <br></br><input style = 'color:#FFFFFF; background-color: #808080; height: 30px; margin-left:440px;' type="submit" value="Lookup"></input></p>
</form>
<?php
